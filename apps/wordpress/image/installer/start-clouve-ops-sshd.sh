...
#      identically on this service, wordpress-mariadb, and the magneto-agent
#      container — hard-coded in dev compose, generated by Clouve as a
#      `secret` in prod). Apply it to the clouve-ops user via chpasswd.
...
#      (apache2-foreground / mariadbd) exits, sshd is reaped along with it.
#
