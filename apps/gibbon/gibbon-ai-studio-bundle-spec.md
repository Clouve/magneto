# Spec: Pre-bundled AI Studio (Claude Code) + Gibbon with a Gibbon DevOps Skill

## 1. Objective

Produce a **pre-bundled Clouve marketplace offering** that ships three things as one cohesive unit — **without modifying the stock `@apps/ai-studio` container image**:
...
2. A new **Gibbon DevOps Skill**, **side-loaded into the container via a volume mount** at a fixed path (`/clouve/skills/gibbon-devops/`) and symlinked into the per-tenant user's `~/.claude/skills/` during session init. The skill is the only new Clouve-authored code in the bundle.
3. `@apps/gibbon` — a running Gibbon (gibbonedu) instance deployed alongside AI Studio, reachable over the pod-internal network via the service name `gibbon` (as already used in [bundles/education-kit/clv-docker-compose.yml](../../bundles/education-kit/clv-docker-compose.yml) via the `containerReference` env-var type).
...
The user experience target is: **a school administrator logs into Clouve, launches this bundle, drops in their Anthropic API key, and has a live Gibbon instance plus a Claude Code agent that can safely evolve, patch, upgrade, and troubleshoot Gibbon — through natural language, without breaking production.**

The entire project, architecturally, is: **stock AI Studio container + stock Gibbon container + stock `gibbon-mysql` container + a skill mounted via ConfigMap + a bit of Compose-level glue at `bundles/gibbon-ai-studio/clv-docker-compose.yml`.** The skill is where all the interesting work lives. The composition pattern is the one already proven by [bundles/education-kit/](../../bundles/education-kit/).
...
- Gibbon is a *real* production workload — school data, gradebooks, attendance — so getting the safety rails right here proves the pattern for future bundles (WordPress, Moodle, Odoo, etc.).
- The Gibbon DevOps Skill is a template. If we can ship one, we can ship N.

The final plan should explicitly call out what in this work is **Gibbon-specific** vs. what is **reusable skill/bundle scaffolding** for future apps.
...
### 3.1 Research artifacts (Phase 1)

A single document (living under `@apps/gibbon/docs/` or the skill folder) that captures everything Claude Code needs to know to run Gibbon safely. It must be sourced from the three canonical locations:

- **Local shallow clone of [GibbonEdu/core](https://github.com/GibbonEdu/core)** at the release we ship (currently `v30.0.01` per the most recent `apps/gibbon` upgrade). Used as a *research input only* — not vendored into the monorepo, not added as a submodule, just `.gitignore`'d at a scratch path so the research phase can grep the real installer, upgrade scripts, `module.php` manifests, and schema SQL instead of paraphrasing docs. Critical for extracting the concrete list of tables for the "never touch" / "safe to touch" split in §3.1. The skill itself must reference upstream URLs + the authored-against version, not code internals that may drift in 31.x.
...
At minimum, the research must cover:
...
### 3.2 The Gibbon DevOps Skill (Phase 2)

A skill folder following the SKILL.md + progressive-disclosure pattern we already use internally. The skill must live **outside `@apps/ai-studio`** (since we don't touch that codebase) and **outside `@apps/gibbon`** (since the skill is bundle-scoped, not gibbon-app-scoped). The monorepo's existing conventions are `apps/<name>/` for single apps and `bundles/<name>/` for compositions; a skill is neither. Recommended layout: place the skill next to the bundle manifest at `bundles/gibbon-ai-studio/skill/` so it travels with the bundle it belongs to, and introduce this as a reusable pattern (`bundles/<name>/skill/`) for future AI-Studio-powered bundles. The bundle's `clv-docker-compose.yml` references this folder as the source for the ConfigMap / volume that gets projected into the AI Studio container at `/clouve/skills/gibbon-devops/` (see §5.1 for why this path and how it reaches the user's `~/.claude/skills/`).

Minimum contents:

- `SKILL.md` — the entry point. Must include:
  - **Description** (used for skill triggering): scoped tightly enough that it only fires for Gibbon-related ops, not generic PHP/MySQL questions. The plan should propose exact wording.
...
- `reference/` — the research artifacts from 3.1, split into focused files so Claude Code loads only what it needs (e.g., `upgrade.md`, `modules.md`, `backup-restore.md`, `troubleshooting.md`, `data-model.md`).
- `playbooks/` — step-by-step procedures for the most common live-server tasks. Each playbook is a named, idempotent-where-possible recipe. Starter set:
  - `upgrade-gibbon.md`
  - `install-module.md`
...
- `scripts/` — small, auditable helpers the agent is allowed to invoke: `backup.sh`, `verify-health.sh`, `php-info.sh`, etc. These are the *only* paths through which Claude Code should perform destructive actions; freehand `rm`/`DROP` must be discouraged by the skill text.

The skill must be explicit about **safety gates**:
...
### 3.3 Bundling mechanics (Phase 3)

No new images. No fork of AI Studio. The bundle is a **composition**: stock containers + a volume-mounted skill + deployment glue. The plan must specify:

- **Container set.** Stock `ai-studio` image (unchanged) from [apps/ai-studio/](../ai-studio/), Gibbon image from [apps/gibbon/](../gibbon/) (which we already build — see `image/build.config`), and the `gibbon-mysql` image from [apps/gibbon/image/mysql/](../gibbon/image/mysql/). All three are already shipped as separate marketplace apps; the bundle is a composition manifest, not a new build. Composed via the existing Docker Compose + Clouve metadata pattern (see [bundles/education-kit/clv-docker-compose.yml](../../bundles/education-kit/clv-docker-compose.yml) as the canonical precedent for a three-container bundle).
- **Skill mount (the core mechanism).** The Gibbon DevOps Skill folder is delivered into the AI Studio container as a **volume mount** at the fixed path `/clouve/skills/gibbon-devops/` (inside the image layer hierarchy that the AI Studio entrypoint already uses for its own installer assets at `/clouve/ai-studio/installer/`). This path is **not** on any persistent volume (`/usr`, `/var`, `/opt`, `/home` are volumes; `/clouve/` is image-layer + mount target), so the skill is genuinely ephemeral and re-projected from source on every pod start — exactly the behaviour we want for skill-update-by-redeploy. Delivery options, in order of preference:
...
A written test matrix proving the bundle actually works. Must include at least:
...
- Marketplace listing copy for the bundle (positioning, target buyer = schools, what's included).
- Tenant-facing "first 10 minutes" guide.
- Internal runbook for Clouve ops: how to debug a tenant's bundle when something goes wrong at a layer Claude Code can't see.
...
- This bundle targets **single-tenant-per-instance** (one school per launched bundle). Multi-school-on-one-Gibbon is explicitly out of scope for v1.
- No data migration *into* the bundle from external SISes in v1. Fresh installs only.
...
3. Phased execution plan with concrete tasks, owners, and rough effort (S/M/L).
4. Skill design (file tree + one-paragraph purpose for each file).
5. Research outline (the headings the research doc will fill — not the research itself yet).
6. Bundling mechanics (the decisions from §3.3, with chosen options and rationale).
...
10. Suggested first PR: the smallest vertical slice that ends with **"a stock AI Studio container, launched with `AI_STUDIO_CLIENT=claude-code` and a volume-mounted Gibbon DevOps Skill folder (containing only a stub `SKILL.md` at this stage) at `/clouve/skills/gibbon-devops/` plus a `/etc/profile.d/clv-gibbon-skill.sh` drop-in that symlinks it into `$HOME/.claude/skills/`, successfully surfaces that skill when Claude Code is asked to introspect its skills after the user opens `/_clv/chat` and completes the first-run API-key prompt."** Gibbon itself doesn't need to be in this slice. §5.1 is already resolved on paper; this PR exists to *prove the mechanism works end-to-end* inside a real AI Studio pod before we stack playbooks, research, and Gibbon on top.
...
- The bundle's "fresh launch → tenant drops in API key → working Gibbon + skill-aware Claude Code" path is described end-to-end without handwaving.
- It is explicit throughout the plan that **AI Studio is not modified** and the skill is the only Clouve-authored new code in this bundle.

---

*Inputs for the research phase:*
- Shallow clone of `https://github.com/GibbonEdu/core` at the shipped release tag (research input only — gitignored, not vendored)
